...
| `static/img/og_*.jpeg` | The eight social preview images shown when a page link is shared (a big icon on a teal, mustard or dark panel with the v3 logo on the object, generated by the `logo-designer` skill). They are site furniture, not artwork to reuse in a poster. |
